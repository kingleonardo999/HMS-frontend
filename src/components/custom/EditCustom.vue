<template>
  <el-drawer size="20%" v-model="drawer" :title="isEditing ? '修改顾客' : '添加顾客'" direction="rtl"
    ...
    <el-form size="small" ref="formRef" :model="formData" status-icon :rules="rules" label-width="70px">
      ...
      <el-form-item label="房间类型" prop="roomTypeId">
        <el-select v-model="formData.roomTypeId" placeholder="请选择房间类型" @change="loadRoomList" :disabled="isEditing">
      ...
      <el-form-item v-if="!isEditing" label="入住日期" prop="resideDate">
        <div class="block">
          <el-date-picker v-model="formData.resideDate" type="datetime" placeholder="请选择入住日期" style="width: 100%" />
        </div>
      </el-form-item>
      <el-form-item v-if="isEditing" label="退房日期" prop="leaveDate">
        <div class="block">
          <el-date-picker v-model="formData.leaveDate" type="datetime" placeholder="请选择退房日期" style="width: 100%" />
        </div>
      </el-form-item>
      <el-form-item v-if="!isEditing" label="押金" prop="deposit">
...
import { $add, $getRoomList, $update } from '../../api/guest';
...
const rules = reactive<FormRules<typeof formData>>({
  ...
  resideDate: [{ required: true, message: '请选择入住日期', trigger: 'change' }],
  deposit: [{ required: true, validator: validateDeposit, trigger: 'blur' }],
